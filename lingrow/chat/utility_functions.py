...
def get_user_group_chats(request):
    ...
    if user.is_teacher():
        ...
        for group in TeacherGroupChat.objects.all().filter(group__teacher__id__exact=teacher.id): groups.append(group.id_chat)
        if teacher.school:
        ...
        for group in ParentGroupChat.objects.all().filter(Q(group__owner=user)): groups.append(group)
        ...
        for group in TeacherGroupChat.objects.all().filter(Q(group__owner=user) | Q(group__owner__isnull=True)): groups.append(group)
        for group in ParentGroupChat.objects.all().filter(Q(group__owner=user) | Q(group__owner__isnull=True)): groups.append(group)
